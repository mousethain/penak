...
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(${REPO}access/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
function base_package() {
    ...
    apt install zip pwgen openssl netcat socat cron bash-completion netcat-openbsd haproxy -y
    apt update -y
    apt upgrade -y
    apt dist-upgrade -y
...
rm -f /root/cf.sh
    CHATID="1393986461"
    KEY="6347044607:AAFXKGZpDWlcmJWrSkxHrjm47FdrsMKKP14"
    TIME="10"
